...
# Find the latest Ubuntu LTS AMI (futureproof, matches ssd and ssd-gp3, always picks latest xx.04)
AMI=$(aws ec2 describe-images --owners 099720109477 \
  --filters 'Name=name,Values=ubuntu/images/hvm-ssd*/ubuntu-*-amd64-server-*' 'Name=state,Values=available' \
  --region $AWS_REGION --profile $AWS_PROFILE \
  --query "Images[?contains(Name, '.04')].{Name:Name,ImageId:ImageId}" \
  --output json | \
  jq -r '.[] | select(.Name | test("ubuntu-[a-z]+-[0-9]{2}\\.04-")) | [.Name, .ImageId] | @tsv' | \
